...
class Enigma:
    ...
    def encrypt(self, message):
        ...
        for c in message:
            encrypt_char = self.encrypt_char(c)
...
def main():
    ...
    parser.add_argument('-o', '--output', help= "Path to the output file to write encryptes messages. If not provided, messages will be printed to the console.")
    ...
    try:
        with open(args.input, 'r') as input_file:
            ...
            for line in input_file:
                line = line.strip()
                encrypted_messages = enigma_machine.encrypt(line)
                encrypted_messages.append(encrypted_messages)

        if args.output:
            with open(args.output, 'w') as output_file:
                for message in encrypted_messages:
                    output_file.write(message + '/n')
        else:
            for message in encrypted_messages:
                print(message)
